			...
			<ul>
				...
				<li>BOOKING</li>
						  ...
						  <table id="example2" class="table table-bordered table-hover" border="2">
						    <thead>
							<tr><th>Train Name</th><th>Sleeper Class</th><th>AC First class</th><th>AC 2 TIER</th><th>AC 3 TIER</th><th>Total Seat</th><th>Action</th></tr>
							</thead>
							  <?php
                           include("connection.php");
						...
						$tid=$_GET['X'];
							...
							$sel="SELECT `trainname`,`sleeper`,`ac1`,`ac2`,`ac3`,`total`, `trainid`  FROM `tbl_train` WHERE `trainid` = '$tid'";
							...
							while($row=$res->fetch_assoc()){
									...
									echo "<tr><td>" .$row["trainname"]. "</td><td>" .$row["sleeper"]. "</td><td>" .$row["ac1"]. "</td><<td>" .$row["ac2"]. "</td><td>" .$row["ac3"]. "</td><td>" .$row["total"]. "</td><td><form method='post' action='booking3.php?X=".$row["trainid"]."'><input type='hidden' name='h1' value=" .$row["trainid"]. "><input type='submit' name='book' value='BOOK'></form></td></tr>";
